...
if(isset($_SESSION['payID'])){
    $refNum = $_SESSION['payID'];
   $stdCod= $_SESSION['userID'];
    $std=$_SESSION['email'];
    
    // make a query
$sql ="SELECT RefNumber, matricID, CourseCode, AmountPaid, PayType from studentPayments  WHERE RefNumber  ='$refNum'";
$records = mysqli_query($con,$sql);
$query="Select DatePaid From studentPayments where studentEmail ='$std'";
...
$pdf->cell(40,10,"ReciptNumber", 1, 0,'C');
...
while ($row=mysqli_fetch_array($records)){
	...
	$pdf->cell(40,10, $row['RefNumber'], 1,0,'C');
    $pdf->cell(40,10, $row['matricID'], 1,0,'C');
    $pdf->cell(50, 10, $row['PayType'], 1,0, 'C');
    $pdf->cell(80,10, $row['CourseCode'], 1,0,'C');
