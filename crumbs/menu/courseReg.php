...
if(isset($_POST['addCourses'])){
 ...
 // ======= check Code Availability =====
        ...
        $query = "SELECT CourseCode  FROM studentCourseReg WHERE MatricNumber ='$CourseCode'";
        $query = mysqli_query($con,$query);
        $CCode = mysqli_num_rows($query); 

            if($CCode == 1){
                $msg = "<script> alert('Course Already Added By You'); </script>";
    } else {
...
$userPayID="";
$stdEmail ="";
$query = "SELECT RefNumber, studentEmail  FROM studentPayments WHERE studentEmail ='$stdEmail' LIMIT 1";
        $query = mysqli_query($con,$query);
        $pCode = mysqli_num_rows($query); 
        if($pCode == 1){
 ...
 VALUES ('$MatricNumber','$CourseCode', '$CourseName','$Semester','$faculty',
        ...
        $query = "SELECT CourseCode  FROM studentCourseReg WHERE MatricNumber ='$MatricNumber' AND CourseCode ='$CourseCode'";
        $query = mysqli_query($con,$query);
        $CCode = mysqli_num_rows($query); 
        ...
        $msg="You Have not paid your school fees";
    }
    }
}
                                           ...
                                           <span style="color:green; font-size:18px;"><?php if(isset($msg)){ echo $msg; } ?></span>
                                            ...
                                            <div class="form-group">
                                                <label class="small mb-1" for="CourseName">Courses Name</label>
                                                 <select name="CourseName" class="form-control py-4" id="CourseName" size="1">
