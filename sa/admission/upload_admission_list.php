            ...
            */ 
              /* $filename= explode(".", $_FILES['file']['name']);
               if( end($filename) == "xlsx" ){
                   ...
                   move_uploaded_file($handle,$path.$filename);
                   while($data=fgetcsv($handle)){
                       $reg = mysqli_real_escape_string($con,$data[0]);
                       $course = mysqli_real_escape_string($con,$data[1]);
                       $query = "UPDATE dml SET course='$course' Where RegNumber ='$reg'";
                       $adm=mysqli_query($con, $query);
                       //print_r($data);
                       
                   }
                  
                   fclose($handle);
                   header('location:https://eportal.escohsti-edu.ng/staffArea/admission/upload_admission_list.php?msg=IT_SUPPORT && updation=1');
               }
    
           }else{
               $msg='<label class="text-danger"> Please Select a File to Upload</label>';
           }*/
            
           /* if($mess){
                
                       echo  '<script> alert ("Admission Data imported Successfully");</script>';
                    header("location:dashboard.php?msg=You Have Successfully Imported Admission List Data");
            }*/
...
if(isset($_GET['updation'])){
    $msg='<label class="text-success"> Admission List Uploaded and  Updated Successfully </label> '.$course;
}
        ...
        if($con){
            ...
            foreach ($xlsx->rows() as $k => $r) {
                ...
                if (!empty($set_errors)) {
                    ...
                    $coH = $header[1];
                    $coV = $val[1];
                    $check_dataExist = getadmission($con, $regH, $regV, $tblName);
                    if($check_dataExist != $val[2]){
                        $updateData = updateadmission($con, $regH, $regV, $coH, $coV, $tblName);
                        $inserted = true;
                    }
                    elseif(!$check_dataExist || empty($check_dataExist)){
                        $sql = "INSERT INTO $tblName (";
                		$i = 0;
                		foreach($header as $k => $v)
                		{   
            ...
            if($inserted){
                echo  '<script> alert ("Admission Data imported Successfully");</script>';
                header("location:dashboard.php?msg=You Have Successfully Imported and Updated Admission Data");
            }elseif($inserted){
                echo  '<script> alert ("Admission Data imported Successfully");</script>';
                header("location:dashboard.php?msg=You Have Successfully Imported and Updated Admission Data");
            }else{
                echo  '<script> alert ("No change in file or wrong file data");</script>';
            }
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="card shadow-lg border-0 rounded-lg mt-5">
                                    ...
                                    <div class="card-header"><h3 class="text-center font-weight-light my-4">Import / Update Admission Data List</h3></div>
                                    <div class="card-body">
                                        <form  action="upload_admission_list.php" method="POST" enctype="multipart/form-data">
                                            <div class="form-group">
                                                <label class="small mb-1" for="inputfilename"> File Name:</label> <?php echo  $msg; ?>
                                                <input class="form-control py-4" id="inputEmailAddress" type="file" name="file"  />
                                           </div>
                                            ...
                                            <!--<div class="form-row">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="small mb-1" for="inputLastName">Faculty</label>
                                            ...
                                            <div class="form-group mt-4 mb-0"><button type="submit" name="submit" class="btn btn-info">IMPORT DATA</a></div>
                                        </form>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-striped">
                                            ...
                                            while($row=mysqli_fetch_array($results)){
                                                echo "<tr>
                                                <td>".$row['RegNumber'] ."</td> <td>".$row['course']."</td></tr>";
                                            }
                                            ?>
                                        </table>
