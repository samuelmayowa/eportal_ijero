...
if (count($students) > 0) {
    foreach ($students as $row) {
     ...
     //====== Start Application Form Report
     elseif($rtype=='Reg' && $dept != 'All'){
         
         
$query = "SELECT RegNumber, FirstName, MiddleName, Surname, PhoneNO, Email, Schools, Departments,StOrigin,LGA, Address,Gender,Username,Amount,Invoices.RefNum FROM onlineApplications JOIN Invoices Using (StudentID) Where onlineApplications.Departments ='$dept2' AND Invoices.ServiceType='2020/2021 Application Form' AND Invoices.InvoiceStatus='Successful'  ORDER BY Departments";
if (!$result = mysqli_query($con, $query)) {
    exit(mysqli_error($con));
}  
$students = array();
...
header('Content-Type: text/csv; charset=utf-8');
header('Content-Disposition: attachment; filename=Registration  Report.csv');
$output = fopen('php://output', 'w');
fputcsv($output, array('RegNumber', 'FirstName', 'MiddleName', 'Surname', 'PhoneNO', 'Email', 'Schools', 'Departments','StOrigin','LGA',' Address','Gender','Username','Amount Paid','RRR NUMBER'));
...
if (count($students) > 0) {
    ...
    }elseif($dept =='All' && $rtype=='Reg'){
      $dpt = $_SESSION['dpt'];
$query = "SELECT RegNumber, FirstName, MiddleName, Surname, PhoneNO, Email, Schools, Departments,StOrigin,LGA, Address,Gender,Username,Amount,Invoices.RefNum FROM onlineApplications JOIN Invoices Using (StudentID) Where Invoices.ServiceType='2020/2021 Application Form' AND Invoices.InvoiceStatus='Successful'";
if (!$result = mysqli_query($con, $query)) {
    exit(mysqli_error($con));
}  
$students = array();
...
header('Content-Type: text/csv; charset=utf-8');
header('Content-Disposition: attachment; filename=All Registration Report.csv');
$output = fopen('php://output', 'w');
fputcsv($output, array('RegNumber', 'FirstName', 'MiddleName', 'Surname', 'PhoneNO', 'Email', 'Schools', 'Departments','StOrigin','LGA',' Address','Gender','Username','Amount Paid','RRR NUMBER'));
...
if (count($students) > 0) {
    ...
    // ============== School Fees reprot for 2021/2022 ==============
    
    
    elseif($rtype=='School' && $dept != 'All'){
         
    // $query = "SELECT * FROM studentPayments"; 
     $query = "SELECT PayID,  matricID, PayType, AmountPaid, RefNumber, Amountpayable, DatePaid, StdLevel, AcademicSession FROM studentPayments"; 
//$query = "SELECT RegNumber, FirstName, MiddleName, Surname, PhoneNO, Email, Schools, Departments,StOrigin,LGA, Address,Gender,Username,Amount,Invoices.RefNum FROM onlineApplications JOIN Invoices Using (StudentID) Where onlineApplications.Departments ='$dept2' AND Invoices.ServiceType='2020/2021 Application Form' AND Invoices.InvoiceStatus='Successful'  ORDER BY Departments";
if (!$result = mysqli_query($con, $query)) {
    exit(mysqli_error($con));
}  
...
header('Content-Type: text/csv; charset=utf-8');
header('Content-Disposition: attachment; filename=School Fees  Report.csv');
$output = fopen('php://output', 'w');
// fputcsv($output, array('RegNumber', 'FirstName', 'MiddleName', 'Surname', 'PhoneNO', 'Email', 'Schools', 'Departments','StOrigin','LGA',' Address','Gender','Username','Amount Paid','RRR NUMBER'));
fputcsv($output, array('PayID',  'matricID', 'PayType', 'AmountPaid', 'RefNumber', 'Amountpayable', 'DatePaid', 'StdLevel', 'AcademicSession'));
if (count($students) > 0) {
    foreach ($students as $row) {
        fputcsv($output, $row);
    }
}
    }elseif($dept =='All' && $rtype=='School'){
      //$dpt = $_SESSION['dpt'];
    //   $query = "SELECT * FROM studentPayments"; 
     $query = "SELECT PayID, matricID, PayType, AmountPaid, RefNumber, Amountpayable, DatePaid, StdLevel, AcademicSession  FROM studentPayments"; 
//$query = "SELECT RegNumber, FirstName, MiddleName, Surname, PhoneNO, Email, Schools, Departments,StOrigin,LGA, Address,Gender,Username,Amount,Invoices.RefNum FROM onlineApplications JOIN Invoices Using (StudentID) Where Invoices.ServiceType='2020/2021 Application Form' AND Invoices.InvoiceStatus='Successful'";
...
header('Content-Type: text/csv; charset=utf-8');
header('Content-Disposition: attachment; filename=All School Fees Report.csv');
$output = fopen('php://output', 'w');
//fputcsv($output, array('RegNumber', 'FirstName', 'MiddleName', 'Surname', 'PhoneNO', 'Email', 'Schools', 'Departments','StOrigin','LGA',' Address','Gender','Username','Amount Paid','RRR NUMBER'));
fputcsv($output, array('PayID',  'matricID', 'PayType', 'AmountPaid', 'RefNumber', 'Amountpayable', 'DatePaid', 'StdLevel', 'AcademicSession'));
if (count($students) > 0) {
