...
if(isset($_FILES['stdData']['name'])){
    ...
    include("SimpleXLSX.php");
        if($con){
            ...
            if($mess){
                $query11 = mysqli_query($con,"SELECT * FROM ".$tblName."");
                       while($student_data = mysqli_fetch_array($query11)){
                    	    $id = $student_data['ID'];
                    	    $state_origin = $student_data['stateOfOrigin'];
                            $citizen = $student_data['Citizenship'];
                            $level = $student_data['studentLevel'];
                            $curlevel = $level[0].$level[1].$level[2];
                            
                    	    if(strtoupper(trim($state_origin)) =='EKITI' && $citizen !="INDIGENE"){
                                $citizen = "INDIGENE";
                                $state_origin = strtoupper(trim($state_origin));
                            }
                            elseif(strtoupper(trim($state_origin)) != 'EKITI' && $citizen !="NON-INDIGENE"){
                                 $citizen = "NON-INDIGENE";
                                 $state_origin = strtoupper(trim($state_origin));
                            }
                            $query = mysqli_query($con, "UPDATE ".$tblName." SET Citizenship ='$citizen', stateOfOrigin = '$state_origin', studentLevel = '$curlevel' where ID ='$id'");
            ...
            <ul class="navbar-nav ml-auto ml-md-0" style="color:#fff;">
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="userDropdown">
                        ...
                        <a class="dropdown-item" href="#">Got Back</a>
                                    ...
                                    <div class="card-body">
                                        <form action="uploadStudents.php" method="POST" enctype="multipart/form-data">
                                            <div class="form-group">
                                                ...
                                                <input class="form-control py-4" id="inputEmailAddress" type="file" name="stdData"  />
                                           </div>
...
/*if(isset($_FILES['stdData']['name'])){
    ...
    include("SimpleXLSX.php");
        ...
        $std=SimpleXLSX::parse($_FILES['stdData']['tmp_name']);
        echo "<pre>"; 
        //print_r($std->rows(1));
       // print_r($std->dimension(2));
        //print_r($std -> sheetNames());
