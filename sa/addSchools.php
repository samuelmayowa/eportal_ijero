...
if(!isset($_SESSION['userID'])){
  confirmLogin();
    ...
    //prepare Data
if(isset($_POST['addSchools'])){
     $screenData = filter_var_array($_POST, FILTER_SANITIZE_STRING);
     $facultyID = $screenData['schoolID'];
     $school= $screenData['school'];
     $deptUnits= $screenData['deptNum'];
      $DateEst= $screenData['DateEst'];
 ...
 // ======= check Code Availability =====
        ...
        $query = "SELECT DeptID FROM Faculties WHERE FacultyID ='$facultyID'";
        $query = mysqli_query($con,$query);
        $CCode = mysqli_num_rows($query); 

            if($CCode == 1){
                $msg = "<script> alert('1 School Already Exist'); </script>";
    } else {
   

//Append Courses

 $addCourse = "INSERT INTO Faculties(FacultyID, FacultyName, DeptUnits, DateCreated) 
 VALUES ('$facultyID', '$school', '$deptUnits', '$DateEst')";
       
        // ======= check Code Availability =====
        
        $query = "SELECT FacultyID FROM Faculties WHERE FacultyID ='$DeptID'";
        $query = mysqli_query($con,$query);
        $CCode = mysqli_num_rows($query); 
        ...
        if(!$addCourse){
            ...
            $msg="<script> alert('1 School Added Successfully'); </script>";
            $msg="1 School Added Successfully";
        }
    }
    else{
        $msg = "<script> alert('1 School Already Exist'); </script>";
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="card shadow-lg border-0 rounded-lg mt-5">
                                    ...
                                    <div class="card-header"><h3 class="text-center font-weight-light my-4"> Manage Schools(Faculties)</h3></div>
                                        ...
                                        <form action="addSchools.php" method="POST" enctype="multipart/form-data">
                                           <span style="color:green; font-size:18px;"><?php if(isset($msg)){ echo $msg; } ?></span>
                                            <div class="form-row">
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input class="form-control py-4" id="inputFirstName" type="text" placeholder="School ID" name="schoolID" required />
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="small mb-1" for="inputFirstName">Faculty Name</label>
                                                        <input class="form-control py-4" id="inputFirstName" type="text" placeholder="Faculty Name"  name="school" required  />
                                                    </div>
                                                </div>
                                               </div>
                                            ...
                                            <div class="form-row">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="small mb-1" for="inputFirstName">Number Of Departments(In The Faculty) </label>
                                                        <input class="form-control py-4" id="inputFirstName" name="deptNum" type="text" placeholder="Number Courses" required />
                                                    </div>
                                                </div>
                                           <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="small mb-1" for="inputFirstName"> Date Created </label>
                                                        <input class="form-control py-4" id="inputFirstName" type="text" placeholder="Date Created in YYYY-00-000"  name="DateEst" required />
                                                    </div>
                                                </div>
                                               </div>
                                          
                                            <div class="form-group mt-4 mb-0"><button type="submit" name="addSchools" class="btn btn-primary btn-block">Add Faculty</a></div>
                                             </div>
                                        </form>
                                    </div>
