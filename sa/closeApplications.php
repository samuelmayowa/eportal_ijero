...
if(isset($_POST['sartAppl'])){
     $screenData = filter_var_array($_POST, FILTER_SANITIZE_STRING);
     $sess = $screenData['sess'];
     if($sess=='2021/2022'){
          $sql_department = "UPDATE stds_bkp SET studentLevel ='100' WHERE  ID=773";
                                                $department_data = mysqli_query($con,$sql_department) or die(mysqli_error($con));
                        if(mysqli_affected_rows($con)==1) {
                            $msg="Session Started Successfuly";
                            echo "<script>alert('$msg'); </script>";
                        }
     } elseif($sess=='2022/2023'){
          $sql_department = "UPDATE stds_bkp SET studentLevel ='200' WHERE  ID=773";
                                                $department_data = mysqli_query($con,$sql_department) or die(mysqli_error($con));
                        if(mysqli_affected_rows($con)==1) {
                            $msg="Session Started Successfuly";
                            echo "<script>alert('$msg'); </script>";
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="card shadow-lg border-0 rounded-lg mt-5">
                                    ...
                                    <div class="card-header"><h3 class="text-center font-weight-light my-4"> START / CLOSE APPLICATION </h3></div>
                                           ...
                                           <span style="color:green; font-size:18px;"><?php if(isset($msg)){ echo $msg; } ?></span>
                                               ...
                                               <div class="form-row">
                                                <div class="col-md-6">
                                                  ...
                                                  <div class="col-md-6">
                                                    <div class="form-group">
                                          <div class="form-control py-4" ><button type="submit" name="sartAppl" class="btn btn-primary btn-block" onlick="button.attr:disable;">CLOSE / START APPLICATION</div>
                                            </div>
                                            </div>
                                             </div>
                                        </form>
