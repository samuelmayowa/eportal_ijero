...
<?php
  ...
  while($row = $result->fetch_assoc()){
    $courseCode[] = array("id" => $row['CourseCode'], "val" => $row['CourseCode']);
  }

  $query = "SELECT CourseTitle, CourseUnits FROM Courses where CourseCode ='$courseCode'";
  $result = mysqli_query($con,$query);

  while($row = $result->fetch_assoc()){
    $courseUnits[$row['CourseUnits']][] = array("id" => $row['CourseUnits'], "val" => $row['CourseTitle']);
  }

  $jsonCourseCode = json_encode($courseCode);
  $jsonCourseUnits = json_encode($courseUnits);
...
<html lang="en">
    ...
    <script type='text/javascript'>
      <?php
        echo "var categories = $jsonCourseCode; \n";
        echo "var subcats = $jsonCourseUnits; \n";
      ?>
                                            ...
                                            <div class="form-group">
                                                <label class="small mb-1" for="inputEmailAddress">Courses Name</label>
                                                 <select name="CourseName" class="form-control py-4" id="inputCourseName" size="1">
                                                            <option value="Select School">....Select Course Name...</option>
                                                 <?php 
                                                            //getCourseCode();
                                                                        $schlID ='';

                                                            $query = "SELECT  Coursetitle, CourseUnits FROM Courses";
                                                            $query = mysqli_query($con, $query) or die (mysqli_error($con));
                                                            while ($schl = mysqli_fetch_array($query)){
                                                               $courseUnits = $schl ['CourseUnits'];
                                                               $schools = $schl ['Coursetitle'];
                                                               echo $schools .= "<option value='$schools'>$schools. '  (' . $courseUnits . ' )'. </option>";}  ?>   
                                                               </select>
