...
if(isset($_SESSION['userID'])){
  ...
  //==== Verify student Citizenship ======
    if($query = mysqli_query($con, "Select stateOfOrigin, Citizenship From students Where matricNumber ='$user'")){
        while ($result = mysqli_fetch_array($query)){
            $st = $result['stateOfOrigin'];
            $citizen = $result['Citizenship'];
        }
        //$msg =$citizen;
        if($st =='EKITI' && ($citizen !="INDIGENE")){
            if($query = mysqli_query($con, "UPDATE students SET Citizenship ='INDIGENE' where stateOfOrigin ='EKITI'")){
              $msg="Your Citizenship is Set By Default To Indigene"  ;
        }
    }
    elseif($st !='EKITI' && ($citizen !="NON-INDIGENE")){
         if($query = mysqli_query($con, "UPDATE students SET Citizenship ='NON-INDIGENE' where stateOfOrigin !='EKITI'")){
              $msg="Your Citizenship is Set By Default to None Indigene"  ;
        }
    }
}
...
//=========== Get State Local Govts for the User ===============
$stID="";
$stID = getStates($con,$st);
if(!empty($stID)){
    $stmsg= $stID;
}
                                           ...
                                           <div class="col-md-6">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input class="form-control py-4" id="inputState" type="text" placeholder="Your State Of Origin"  name="states" required  Value="<?php if(isset($_SESSION['stOrigin'])){ echo $_SESSION['stOrigin']; }  ?>" readonly />
                                                    ...
                                                    <div class="form-group">
                                                      ...
                                                      <!--<input class="form-control py-4" id="inputFirstName" type="text" placeholder="Your Local Govt Area" name="lga" required  Value="<?php if(isset($_POST['lga'])){ echo $_POST['lga']; } ?>" />-->
                                                         <select name="lga" id="lgas"  class="form-control py-4" size="1">
                                                           <?php 
                                                            $stdLocals ="";
                                                            if($query = mysqli_query($con, "Select local_id, local_name From locals Where state_id=$stmsg")){
                                                                    while ($result = mysqli_fetch_array($query)){
...
for (var i=0;i<states.length;i++){
   option += '<option value="'+ states[i] + '">' + $.strtoupper(states[i]) + '</option>';
}
